...
-(void) getNextEvent {
  ...
  {
    double value[25];
    memset(value, 0, 25*sizeof(double));
    int overallResult = EDK_OK;
    
    for(int i=0 ; i< sizeof(ChannelList)/sizeof(IEE_DataChannel_t) ; ++i)
    {
      int result = IEE_GetAverageBandPowers(userID, ChannelList[i], &value[i*5], &value[i*5+1], &value[i*5+2], &value[i*5+3], &value[i*5+4]);
      overallResult = fmax(overallResult, result);
    }

    if(overallResult == EDK_OK){
      [self saveDoubleVal:file data:data value:[[NSDate date] timeIntervalSince1970]];
